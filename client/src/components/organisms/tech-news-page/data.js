export const techBlogData = [
  {
    date: '27 August',
    catagory: 'uncategorized',
    title:
      'Give Your Diesel Engine The Love It Deserves - Proper Maintenance Will Extend Its Service Life',
    text:
      'New technologies are coming to market for on and off-road heavy duty engines but they do not currently have the flexibility or reliability of diesel power. Diesel will continue to be the preferred option for the years ahead and a …',
    image:
      'https://klondikelubricants.com/wp-content/uploads/2021/08/Heavy-Duty-Engine-Oils_Mechanic-Post-03.png',
  },
  {
    date: '09 JULY',
    catagory: 'uncategorized',
    title: 'New Products To Drive Business Growth',
    text:
      'We have launched new oils, lubricants and chemicals to drive business growth and support the range of industries that now rely on KLONDIKE to keep their businesses moving. Download the latest New Products Flyer for more on High Mileage Engine …',
    image:
      'https://klondikelubricants.com/wp-content/uploads/2021/07/New-Products-Thumbnail.png',
  },
  {
    date: '25 june',
    catagory: 'Industry News',
    title:
      'Keep your engine running at a stable temperature with the correct antifreeze/ coolant',
    text:
      'Not using the correct Antifreeze/Coolant can lead to serious issues for your engine. Antifreeze is a temperature control fluid that protects engines from overheating and putting pressure on the radiator and hoses. It also lowers the freezing point of water …',
    image:
      'https://klondikelubricants.com/wp-content/uploads/2021/08/Heavy-Duty-Engine-Oils_Mechanic-Post-03.png',
  },
]
