export const products = [
  {
    category: 'varieties + deals',
    products: [
      {
        id: 0,
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/SamplerMainImage_17e01fb1-7123-423f-b2fa-eb105d6ce96d.png?v=1601044575',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-06.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-08_9a01cca5-7f2d-43c0-965b-2fe68dee4650.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-09_7e29fe26-7c39-438d-ab13-cf2ae30ec979.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/DSC01605.jpg?v=1589569053',
        ],
        title: '7 Bar Sampler',
        price: 14.99,
        subPrice: 10.99,
        size: '7 BARS',
        subscription: false,
        customize: false,
        viewProduct: false,
        currencySymbol: '$',
        swag: false,
        salePrice: 0,
        buttonColor: 'black',
        sizeOptions: [
          { value: 'S', label: 'S' },
          { value: 'M', label: 'M' },
          { value: 'L', label: 'L' },
          { value: 'XL', label: 'XL' },
        ],
        importantNote: 'This item will ship close to Jan 15,2021',
        subscribeItems: [
          {
            id: 0,
            label: 'One-Time Purchase',
            value: 'oneTimePurchase',
            disabled: false,
          },
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
            disabled: false,
          },
        ],
        detailsKey: ['description', 'ingredients', 'netCarbs'],
        product: {
          description: {
            title: 'Description',
            description:
              'This bundle contains 3 boxes: 1 box of 12 Chocolate Sea Salt bars, 1 box of 12 Peanut Butter Chip bars, and 1 box of 12 Almond Butter Chip bars. Satisfy your savory nuts and chocolate dessert cravings without the added guilt!',
          },
          ingredients: {
            title: 'Ingredients',
            ingredients: [
              [
                {
                  title: 'Almond Butter Chip Ingredients',
                  color: '#783F2D',
                  description:
                    "Almonds, Soluble Tapioca Fiber, Pea Protein, Protein Crisps (Pea Protein, Tapioca Starch), Allulose, Chocolate Chips (Chocolate, Allulose, Cocoa Butter, Stevia Extract), Water, Natural Flavors, Gum Acacia, Flaxseeds, Coconut Oil, Sea Salt, Sunflower Lecithin, Stevia Extract, Lion's Mane Extract, Vitamin E",
                },
                {
                  title: 'Almond Butter Chip Allergens',
                  color: '#783F2D',
                  description: 'Almonds, Coconut',
                },
              ],
              [
                {
                  title: 'Chocolate Sea Salt Ingredients',
                  color: '#65493B',
                  description:
                    "Almonds, Soluble Tapioca Fiber, Allulose, Pea Protein, Protein Crisps (Pea Protein, Tapioca Starch), Chocolate Chips (Chocolate, Allulose, Cocoa Butter, Stevia Extract), Water, Cocoa Powder, Natural Flavors, Flaxseeds, Gum Acacia, Cocoa Extract, Coconut Oil, Sea Salt, Sunflower Lecithin, Stevia Extract, Lion's Mane Extract, Vitamin E",
                },
                {
                  title: 'Chocolate Sea Salt Allergens',
                  color: '#65493B',
                  description: 'Almonds, Coconut',
                },
              ],
            ],
          },
          netCarbs: {
            title: 'Nutrients & Carbs',
            images: [
              'https://cdn.shopify.com/s/files/1/1682/9837/t/94/assets/nutritionfacts-abc-desktop.png?v=14108304472421026226',
              'https://cdn.shopify.com/s/files/1/1682/9837/t/94/assets/nutritionfacts-css-desktop.png?v=12550476319516524908',
            ],
          },
        },
        customCaseS: {
          total: 6,
          title: 'Customize(Boxes of 12)',
          flavors: [
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Peanut Butter Chip',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Almond Butter Chip',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 0,
              unit: 'Box',
            },
          ],
        },
      },
      {
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-06.png?v=1590778372',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-06.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-08_9a01cca5-7f2d-43c0-965b-2fe68dee4650.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-08_9a01cca5-7f2d-43c0-965b-2fe68dee4650.png?v=1590778372',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/DSC01605.jpg?v=1589569053',
        ],
        title: 'Custom Case',
        price: 127.45,
        subPrice: 90.99,
        size: 'CASE OF 72',
        subscription: true,
        customize: true,
        viewProduct: false,
        currencySymbol: '$',
        swag: false,
        salePrice: 0,
        buttonColor: 'black',
        sizeOptions: [
          { value: 'S', label: 'S' },
          { value: 'M', label: 'M' },
          { value: 'L', label: 'L' },
          { value: 'XL', label: 'XL' },
        ],
        subscribeItems: [
          {
            id: 0,
            label: 'One-Time Purchase',
            value: 'oneTimePurchase',
            disabled: false,
          },
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
            disabled: false,
          },
        ],
        detailsKey: ['description', 'ingredients', 'netCarbs'],
        product: {
          description: {
            title: 'Description',
            description:
              'This bundle contains 3 boxes: 1 box of 12 Chocolate Sea Salt bars, 1 box of 12 Peanut Butter Chip bars, and 1 box of 12 Almond Butter Chip bars. Satisfy your savory nuts and chocolate dessert cravings without the added guilt!',
          },
          ingredients: {
            title: 'Ingredients',
            ingredients: [
              [
                {
                  title: 'Almond Butter Chip Ingredients',
                  color: '#783F2D',
                  description:
                    "Almonds, Soluble Tapioca Fiber, Pea Protein, Protein Crisps (Pea Protein, Tapioca Starch), Allulose, Chocolate Chips (Chocolate, Allulose, Cocoa Butter, Stevia Extract), Water, Natural Flavors, Gum Acacia, Flaxseeds, Coconut Oil, Sea Salt, Sunflower Lecithin, Stevia Extract, Lion's Mane Extract, Vitamin E",
                },
                {
                  title: 'Almond Butter Chip Allergens',
                  color: '#783F2D',
                  description: 'Almonds, Coconut',
                },
              ],
              [
                {
                  title: 'Chocolate Sea Salt Ingredients',
                  color: '#65493B',
                  description:
                    "Almonds, Soluble Tapioca Fiber, Allulose, Pea Protein, Protein Crisps (Pea Protein, Tapioca Starch), Chocolate Chips (Chocolate, Allulose, Cocoa Butter, Stevia Extract), Water, Cocoa Powder, Natural Flavors, Flaxseeds, Gum Acacia, Cocoa Extract, Coconut Oil, Sea Salt, Sunflower Lecithin, Stevia Extract, Lion's Mane Extract, Vitamin E",
                },
                {
                  title: 'Chocolate Sea Salt Allergens',
                  color: '#65493B',
                  description: 'Almonds, Coconut',
                },
              ],
            ],
          },
          netCarbs: {
            title: 'Nutrients & Carbs',
            images: [
              'https://cdn.shopify.com/s/files/1/1682/9837/t/94/assets/nutritionfacts-abc-desktop.png?v=14108304472421026226',
              'https://cdn.shopify.com/s/files/1/1682/9837/t/94/assets/nutritionfacts-css-desktop.png?v=12550476319516524908',
            ],
          },
        },
        customCaseS: {
          total: 6,
          title: 'Customize(Boxes of 12)',
          flavors: [
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Peanut Butter Chip',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Almond Butter Chip',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 1,
              unit: 'Box',
            },
            {
              flavor: 'Chocolate Sea Salt',
              quantity: 0,
              unit: 'Box',
            },
          ],
        },
      },
    ],
  },
  {
    category: 'single flavor',
    products: [
      {
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-07_00b10e4b-35c0-4291-850c-a494322152b3.png?v=1590778400',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-07_00b10e4b-35c0-4291-850c-a494322152b3.png?v=1590778400',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-10_d551edb9-f6b9-493b-83a7-4ce6fcb430e0.png?v=1590778400',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-11_5817efa0-2bd6-4e0e-8643-61b17c873463.png?v=1590778400',
        ],
        title: 'FRUIT LOVERS BUNDLE',
        price: 67.47,
        subPrice: 30.99,
        size: '7 BARS',
        subscription: true,
        customize: false,
        viewProduct: false,
        swag: false,
        currencySymbol: '$',
        salePrice: 0,
        buttonColor: 'black',
        subscribeItems: [
          {
            id: 0,
            label: 'One-Time Purchase',
            value: 'oneTimePurchase',
          },
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
          },
        ],
      },
      {
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-01.png?v=1590778332',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/bundleswithdiscountbadge-01.png?v=1590778332',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-08_0aef1eee-b2e5-4fed-99cb-9653d5bffce3.png?v=1590778332',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-09_e322fcac-dfda-4fe6-9e72-4c61ea444a43.png?v=1590778332',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-10_a947cb13-8cca-4bee-8edb-edb619e671db.png?v=1590778332',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/AMAZONNUTRITIONFACTS-11_fda9a9fe-5ad2-4ec2-96c0-63357b1e54bd.png?v=1590778332',
        ],
        title: 'CHOCOLATE LOVERS & FRUIT LOVERS',
        price: 47.48,
        subPrice: 20.99,
        size: '24 BARS',
        subscription: true,
        customize: false,
        viewProduct: false,
        swag: false,
        currencySymbol: '$',
        salePrice: 0,
        buttonColor: 'black',
        subscribeItems: [
          {
            id: 0,
            label: 'One-Time Purchase',
            value: 'oneTimePurchase',
          },
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
          },
        ],
      },
    ],
  },
  {
    category: 'swag',
    products: [
      {
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/trucker-hat-1.png?v=1591895205',
        title: 'RUBBER PATCH TRUCKER HAT',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/trucker-hat-1.png?v=1591895205',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/trucker-hat-2.png?v=1591895205',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/DSC01704.jpg?v=1591895205',
        ],
        price: 19.99,
        subPrice: 9.99,
        size: '',
        currencySymbol: '$',
        subscription: false,
        customize: false,
        viewProduct: true,
        swag: true,
        isMultipleSizing: false,
        salePrice: 0,
        buttonColor: 'black',
        subscribeItems: [
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
          },
        ],
        product: {
          description: {
            title: 'Description',
            description: 'Fabric: Cotton blend and trucker mesh combination',
          },
        },
      },
      {
        mainImage:
          'https://cdn.shopify.com/s/files/1/1682/9837/products/crewneck.png?v=1589569243',
        title: 'UNISEX CREWNECK SWEATSHIRT',
        sliderImages: [
          'https://cdn.shopify.com/s/files/1/1682/9837/products/crewneck.png?v=1589569243',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/DSC01589.jpg?v=1589569243',
          'https://cdn.shopify.com/s/files/1/1682/9837/products/DSC01644.jpg?v=1589569243',
        ],
        price: 29.99,
        size: '',
        currencySymbol: '$',
        subscription: false,
        customize: false,
        viewProduct: true,
        swag: true,
        isMultipleSizing: true,
        salePrice: 0,
        buttonColor: 'black',
        subscribeItems: [
          {
            id: 1,
            label: 'Ship Every Week',
            value: 'ShipEveryWeek',
          },
        ],
        sizeOptions: [
          { value: 'S', label: 'S' },
          { value: 'M', label: 'M' },
          { value: 'L', label: 'L' },
          { value: 'XL', label: 'XL' },
        ],
        product: {
          description: {
            title: 'Description',
            description:
              'Fleece-lined, 50% cotton, 50% polyester blend (5% recycled polyester from plastic bottles) blend crewneck sweatshirt.',
          },
        },
      },
    ],
  },
]

export const categoriesXPM = [
  {
    title: 'Nano',
    categoryName: 'Nano',
  },
  {
    title: 'Heavy Duty Engine Oils',
    categoryName: 'Heavy Duty Engine Oils',
  },
  {
    title: 'Automotive Engine Oils',
    categoryName: 'Automotive Engine Oils',
  },
  {
    title: 'Gear Lubricants',
    categoryName: 'Gear Lubricants',
  },
  {
    title: 'Transmission Fluids',
    categoryName: 'Transmission Fluids',
  },
  {
    title: 'Hydraulic Fluids',
    categoryName: 'Hydraulic Fluids',
  },
  {
    title: 'Industrial & Functional Oils',
    categoryName: 'Industrial & Functional Oils',
  },
  {
    title: 'Greases',
    categoryName: 'Greases',
  },
  {
    title: 'Biodegradable Lubricants',
    categoryName: 'Biodegradable Lubricants',
  },
  {
    title: 'Chemicals',
    categoryName: 'Chemicals',
  },
  {
    title: 'Food Grade Lubricants',
    categoryName: 'Food Grade Lubricants',
  },
]
export const filterSelect = [
  {
    name: 'most relevant',
  },
  {
    name: 'price: low to high',
  },
  {
    name: 'price: high to low',
  },
  {
    name: 'alphabetical: a-z',
  },
]
export const size = [
  {
    text: 'size 1',
  },
  {
    text: 'size 2',
  },
  {
    text: 'size 3',
  },
  {
    text: 'size 4',
  },
]
export const partNumber = [
  {
    text: 'Part Number 1',
  },
  {
    text: 'Part Number 2',
  },
  {
    text: 'Part Number 3',
  },
  {
    text: 'Part Number 4',
  },
]
export const unit = [
  {
    text: 'Unit 1',
  },
  {
    text: 'Unit 2',
  },
  {
    text: 'Unit 3',
  },
  {
    text: 'Unit 4',
  },
]
export const untitled = [
  {
    text: 'untitled',
  },
]
export const PlpBottom = [
  {
    image: '/static/images/klondike3.png',
    button: 'DOWNLOAD PDS & MSDS',
    mobileButton: 'How to buy',
  },
  {
    image: '/static/images/klondike3.png',
    button: 'BECOME A DEALER',
    mobileButton: 'lube link',
  },
]
